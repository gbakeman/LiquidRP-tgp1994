...
function SWEP:PrimaryAttack()
	--The below block is copied in from DarkRP's latest base file, to hopefully
	--enforce ironsights
	...
	local nextShoot = CurTime() + self.Primary.Delay
	self.Reloadaftershoot = nextShoot
	-- Set the reload after shoot to be not able to reload when firering
	
	self.Weapon:SetNextSecondaryFire(nextShoot)
	-- Set next secondary fire after your fire delay

	self.Weapon:SetNextPrimaryFire(nextShoot)
	-- Set next primary fire after your fire delay
	
	--Copied in from DarkRP
	if not self:GetNetworkedBool("Ironsights") and GAMEMODE.Config.ironshoot then return end
