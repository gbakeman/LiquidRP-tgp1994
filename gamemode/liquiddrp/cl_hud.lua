...
local IsOpen
LDRP.Buttons = {["More HUD info toggle"] = function(a) LDRP.MoreInfoToggle() end,
["Show health toggle"] = function(a)
	LDRP.ToggleCMD("cl_showhp")
end,
["Show ammo toggle"] = function(a)
	LDRP.ToggleCMD("cl_showammo")
end,
["Show playercard"] = function(a)
	LDRP.ToggleCMD("cl_playercard")
end,
["Show NPC Names"] = function(a)
	LDRP.ToggleCMD("cl_npcnames")
end,
["Close"] = function(a,midy)
	IsOpen = false
	a:MoveTo(ScrW(),midy,.2,0,.5)
...
function LDRP.OptionsMenu()
	if IsOpen then return end
	IsOpen = true
	local OptionsMenu = vgui.Create("DFrame")
	local w,h = ScrW()*.15,ScrH()*.2
	local midy = ScrH()*.5-(h*.5)
	OptionsMenu:SetPos(-w,midy)
	OptionsMenu:SetSize(w,h)
	OptionsMenu:MoveTo(ScrW()*.5-(w*.5),midy,.2,0,.5)
	OptionsMenu:MakePopup()
	OptionsMenu:SetTitle("Options")
	OptionsMenu:ShowCloseButton(false)
	local ButtonSpace = (h-28)/table.Count(LDRP.Buttons)
	
	local Cur = 0
	for k,v in pairs(LDRP.Buttons) do
		local MoreInfoButton = vgui.Create("DButton",OptionsMenu)
		MoreInfoButton:SetPos(4,24+(ButtonSpace*Cur))
		MoreInfoButton:SetSize(w-8,ButtonSpace)
		MoreInfoButton:SetText(k)
		MoreInfoButton.DoClick = function() v(OptionsMenu,midy) end
		Cur=Cur+1
	end
end
...
function LDRP.HUDPaint()
	...
	draw.RoundedBox(6,ScrW()-42,ScrH()-42,40,40, SKIN.control_color ) --Option (tool) button BG
	if !OptionsButton then
		OptionsButton = vgui.Create("DImageButton", myParent)
		OptionsButton:SetMaterial( "icon16/wrench.png" )
		OptionsButton:SetPos(ScrW()-38,ScrH()-38)
	...
	if JustJoined then
		draw.SimpleTextOutlined( "Press F3 and click this button for settings -->","Trebuchet24", ScrW()-44, ScrH()*.98, SKIN.tooltip, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, 2, SKIN.text_outline )
	end
