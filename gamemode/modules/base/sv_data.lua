...
function DarkRP.initDatabase()
	...
	MySQLite.begin()
		...
		MySQLite.queueQuery([[
			CREATE TABLE IF NOT EXISTS ]]..Prefix..[[_jobspawn(
		...
		if MySQLite.CONNECTED_TO_MYSQL then
			MySQLite.queueQuery([[
				ALTER TABLE darkrp_jobspawn ADD FOREIGN KEY(id) REFERENCES ]]..Prefix..[[_position(id)
					ON UPDATE CASCADE
					ON DELETE CASCADE;
			]])
		end
		...
		-- Player information
		MySQLite.query([[
			CREATE TABLE IF NOT EXISTS ]]..Prefix..[[_player(
		...
		-- Door data
		MySQLite.query([[
			CREATE TABLE IF NOT EXISTS ]]..Prefix..[[_door(
		...
		MySQLite.query([[
			CREATE TABLE IF NOT EXISTS ]]..Prefix..[[_jobown(
				idx INTEGER NOT NULL,
				map VARCHAR(45) NOT NULL,
		...
		-- Door groups
		MySQLite.query([[
			CREATE TABLE IF NOT EXISTS ]]..Prefix..[[_doorgroups(
				idx INTEGER NOT NULL,
				map VARCHAR(45) NOT NULL,
				...
				MySQLite.query("SHOW PRIVILEGES", function(data)
					...
					MySQLite.query([[
						CREATE TRIGGER JobPositionFKDelete
							AFTER DELETE ON ]]..Prefix..[[_position
							FOR EACH ROW
								IF OLD.type = "T" THEN
									DELETE FROM ]]..Prefix..[[_jobspawn WHERE ]]..Prefix..[[_jobspawn.id = OLD.id;
								END IF
						;
					]])
				end)
	...
	MySQLite.commit(function() -- Initialize the data after all the tables have been created
		...
		if MySQLite.CONNECTED_TO_MYSQL then -- In a listen server, the connection with the external database is often made AFTER the listen server host has joined,
			...
			for k,v in pairs(player.GetAll()) do
				...
				MySQLite.query([[SELECT * FROM ]]..Prefix..[[_player WHERE uid = ]].. UniqueID ..[[;]], function(data)
...
function DarkRP.storeRPName(ply, name)
	...
	MySQLite.query([[UPDATE ]]..Prefix..[[_player SET rpname = ]] .. MySQLite.SQLStr(name) .. [[ WHERE UID = ]] .. ply:UniqueID() .. ";")
end

function DarkRP.retrieveRPNames(name, callback)
	MySQLite.query("SELECT COUNT(*) AS count FROM "..Prefix.."_player WHERE rpname = "..MySQLite.SQLStr(name)..";", function(r)
...
function DarkRP.retrievePlayerData(ply, callback, failed, attempts)
	...
	MySQLite.query("SELECT rpname, wallet, salary FROM "..Prefix.."_player WHERE uid = " .. ply:UniqueID() .. ";", callback, function()
		DarkRP.retrievePlayerData(ply, callback, failed, attempts + 1)
	end)
end

function DarkRP.createPlayerData(ply, name, wallet, salary)
	MySQLite.query([[REPLACE INTO ]]..Prefix..[[_player VALUES(]] ..
			ply:UniqueID() .. [[, ]] ..
...
function DarkRP.storeMoney(ply, amount)
	...
	MySQLite.query([[UPDATE ]]..Prefix..[[_player SET wallet = ]] .. amount .. [[ WHERE uid = ]] .. ply:UniqueID())
end

local function resetAllMoney(ply,cmd,args)
	if ply:EntIndex() ~= 0 and not ply:IsSuperAdmin() then return end
	MySQLite.query("UPDATE "..Prefix.."_player SET wallet = "..GAMEMODE.Config.startingmoney.." ;")
	for k,v in pairs(player.GetAll()) do
...
function DarkRP.storeSalary(ply, amount)
	...
	MySQLite.query([[UPDATE ]]..Prefix..[[_player SET salary = ]] .. amount .. [[ WHERE uid = ]] .. ply:UniqueID())
...
function DarkRP.retrieveSalary(ply, callback)
	...
	MySQLite.queryValue("SELECT salary FROM "..Prefix.."_player WHERE uid = " .. ply:UniqueID() .. ";", function(r)
...
function DarkRP.storeDoorData(ent)
	...
	MySQLite.query([[REPLACE INTO ]]..Prefix..[[_door VALUES(]]..
		ent:doorIndex() ..[[, ]] ..
		MySQLite.SQLStr(map) .. [[, ]] ..
...
function setUpNonOwnableDoors()
	MySQLite.query("SELECT idx, title, isLocked, isDisabled FROM "..Prefix.."_door WHERE map = " .. MySQLite.SQLStr(string.lower(game.GetMap())) .. ";", function(r)
...
function DarkRP.storeTeamDoorOwnability(ent)
	...
	MySQLite.query("DELETE FROM "..Prefix.."_jobown WHERE idx = " .. ent:doorIndex() .. " AND map = " .. MySQLite.SQLStr(map) .. ";")
	for k,v in pairs(ent:getKeysDoorTeams() or {}) do
		MySQLite.query("INSERT INTO "..Prefix.."_jobown VALUES(" .. ent:doorIndex() .. ", " .. MySQLite.SQLStr(map) .. ", " .. k .. ");")
	end
end

function setUpTeamOwnableDoors()
	MySQLite.query("SELECT idx, job FROM "..Prefix.."_jobown WHERE map = " .. MySQLite.SQLStr(string.lower(game.GetMap())) .. ";", function(r)
		if not r then return end
...
function DarkRP.storeDoorGroup(ent, group)
	...
	if group == "" or not group then
		MySQLite.query("DELETE FROM "..Prefix.."_doorgroups WHERE map = " .. map .. " AND idx = " .. index .. ";")
		return
	end

	MySQLite.query("REPLACE INTO "..Prefix.."_doorgroups VALUES(" .. index .. ", " .. map .. ", " .. MySQLite.SQLStr(group) .. ");");
end

function setUpGroupDoors()
	local map = MySQLite.SQLStr(string.lower(game.GetMap()))
	MySQLite.query("SELECT idx, doorgroup FROM "..Prefix.."_doorgroups WHERE map = " .. map, function(data)
		if not data then return end
