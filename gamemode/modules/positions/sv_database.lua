...
local function onDBInitialized()
	...
	MySQLite.query("SELECT * FROM "..Prefix.."_position NATURAL JOIN "..Prefix.."_jobspawn WHERE map = "..map..";", function(data)
		teamSpawns = data or {}
	end)

	MySQLite.query([[SELECT * FROM ]]..Prefix..[[_position WHERE type = 'J' AND map = ]] .. map .. [[;]], function(data)
...
function DarkRP.setJailPos(pos)
	...
	local remQuery = "DELETE FROM "..Prefix.."_position WHERE type = 'J' AND map = %s;"
	local insQuery = "INSERT INTO "..Prefix.."_position VALUES(NULL, %s, 'J', %s, %s, %s);"
...
function DarkRP.storeTeamSpawnPos(t, pos)
	...
	MySQLite.query([[DELETE FROM ]]..Prefix..[[_position WHERE map = ]] .. MySQLite.SQLStr(map) .. [[ AND id IN (SELECT id FROM ]]..Prefix..[[_jobspawn WHERE team = ]] .. t .. [[)]])

	MySQLite.query([[INSERT INTO ]]..Prefix..[[_position VALUES(NULL, ]] .. MySQLite.SQLStr(map) .. [[, "T", ]] .. pos[1] .. [[, ]] .. pos[2] .. [[, ]] .. pos[3] .. [[);]]
		, function()
		MySQLite.queryValue([[SELECT MAX(id) FROM ]]..Prefix..[[_position WHERE map = ]] .. MySQLite.SQLStr(map) .. [[ AND type = "T";]], function(id)
			if not id then return end
			MySQLite.query([[INSERT INTO ]]..Prefix..[[_jobspawn VALUES(]] .. id .. [[, ]] .. t .. [[);]])
...
function DarkRP.removeTeamSpawnPos(t, callback)
	local map = string.lower(game.GetMap())
	MySQLite.query([[SELECT ]]..Prefix..[[_position.id FROM ]]..Prefix..[[_position
		NATURAL JOIN ]]..Prefix..[[_jobspawn
		...
		for k,v in pairs(data or {}) do
			...
			MySQLite.query([[DELETE FROM ]]..Prefix..[[_position WHERE id = ]]..v.id..[[;]])
		end
		MySQLite.commit()
	end)
